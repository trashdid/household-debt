...
class DebtsService:
    ...
    async def get_state_debt(self, state: str, start_date: datetime, end_date: datetime) -> StateDebt:
        debts = await self.repository.get_state_debt(state, start_date, end_date)

        if len(debts) > 0:
            return debts[0]

        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Debt not found for {state}")
